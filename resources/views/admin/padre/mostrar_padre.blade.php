...
@section('content_header')
    ...
    <div style="text-align: center;">
        <div style="color: white; display: inline-block;">
            <a class="btn btn-primary" href="{{url('crear_padre')}}">crear padre de familia</a>
        </div>
    </div>
...
<div class="div_center">
    <p>Lista de padres de familia</p>
</div>
    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th class="th_deg" scope="col">Parentesco</th>
        ...
        <tbody>
            ...
            <tr>
                <td>{{$padre->CI}}</td>
                <td>{{$padre->Nombre}}</td>
                <td>{{$padre->ApellidoPaterno}}</td>
                <td>{{$padre->ApellidoMaterno}}</td>
                <td>{{$padre->Sexo}}</td>
                <td>{{$padre->Direccion}}</td>
                <td>{{$padre->Telefono}}</td>
                <td>{{$padre->FechaNacimiento}}</td>
                <td>{{$padre->FechaRegistro}}</td>
                <td>{{$padre->Correo}}</td>
                <td>{{$padre->Parentesco}}</td> 

                <td>
                    <div style="display: flex; justify-content: space-between;">
                        <a class="btn btn-warning" href="{{url('actualizar_padre', $padre->id)}}">editar</a>
                        <a class="btn btn-danger" onclick="return confirm('¿Estás seguro?')"
                           href="{{url('borrar_padre', $padre->id)}}">eliminar</a>
                    </div>
                </td>                        
            </tr>
            @endforeach
